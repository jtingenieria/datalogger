...
esp_err_t ds18b20_trigger_temperature_conversion_for_all(ds18b20_device_handle_t any_ds18b20_on_bus)
{
    ESP_RETURN_ON_FALSE(any_ds18b20_on_bus, ESP_ERR_INVALID_ARG, TAG, "invalid argument");
    // reset bus and check if the ds18b20 is present
    ESP_RETURN_ON_ERROR(onewire_bus_reset(any_ds18b20_on_bus->bus), TAG, "reset bus error");

    // send command: DS18B20_CMD_CONVERT_TEMP
    uint8_t tx_buffer[2] = {0};
    tx_buffer[0] = ONEWIRE_CMD_SKIP_ROM;
    tx_buffer[1] = DS18B20_CMD_CONVERT_TEMP;
    return onewire_bus_write_bytes(any_ds18b20_on_bus->bus, tx_buffer, sizeof(tx_buffer));
